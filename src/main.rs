...
pub(crate) const POWERSHELL_PATH: &str =
...
fn plan_for_disk(disk: &PhysicalDisk) -> DiskPlan {
    plan_for_disk_with_capacity_override(disk, false)
}

// Same as plan_for_disk, but capacity_override: true lets a disk on an
// untrusted bus become Planned anyway. This exists for one reason: an
// operator who has independently verified a specific device's true capacity
// outside this tool (main()'s capacity-override prompt) can proceed with it
// -- but the decision is per-invocation and per-operator-acknowledgment, not
// a change to which bus types are trusted in general. Never call this with
// capacity_override: true except in response to that explicit prompt.
fn plan_for_disk_with_capacity_override(disk: &PhysicalDisk, capacity_override: bool) -> DiskPlan {
    match evaluate_eligibility(disk) {
        Eligibility::Blocked(reason) => return DiskPlan::Skipped(reason),
        Eligibility::Eligible => {}
    }

    if !is_capacity_trusted_bus(disk.bus_type.as_deref()) && !capacity_override {
...
// matters only for cost: cheap checks (confirmed_plan shape, confirmation
// code match) run before the more expensive fresh-inventory-dependent ones.
// Called from main()'s confirmation block after the operator's typed line is
// read.
//
// entered_confirmation/expected_confirmation replace an earlier design that
// compared against the disk's own serial number: a hardware serial can be
// near-empty (observed on real hardware: a USB-SD bridge reporting "[" as its
// entire serial), which proves nothing about operator intent. A fresh code
// generated by the caller for this run (see generate_confirmation_code, not
// a security mechanism) is a human proof-of-intent check, independent of how
// weak the hardware's own identity fields are. Disk identity itself is still
// verified below via verify_target_before_operation, unchanged.
fn confirm_and_bind<'a>(
    selected: &PhysicalDisk,
    confirmed_plan: &DiskPlan,
    entered_confirmation: &str,
    ...
    // a cancelled prompt -- can never coincidentally equal the generated code.
    if entered_confirmation.trim() != expected_confirmation {
        return Err("confirmation did not match the generated confirmation code".to_string());
    }
    ...
    // equality (not just "still Planned") catches that. capacity_override
    // must be re-applied identically here, or a capacity-overridden plan
    // could never match its own re-derivation (plan_for_disk alone would
    // always recompute PendingCapacityConfirmation for an untrusted bus).
    let fresh_plan = plan_for_disk_with_capacity_override(fresh, capacity_override);
...
// A short confirmation code that changes on every run. This is NOT a
// security mechanism and DefaultHasher/SystemTime/process id are NOT a
// cryptographic or security-grade random source -- they're just a cheap way
// to make the code differ from one invocation to the next. Its only job is
// to be a fresh human proof-of-intent token: something the operator must be
// looking at *this specific prompt, right now* to copy correctly, unlike a
// static hardware serial number that could in principle be typed from
// memory or a note without reading the current screen -- and which, on real
// hardware seen during this project, can be as degenerate as a single
// character. It is not meant to resist guessing or replay by an adversary.
fn generate_confirmation_code() -> String {
    ...
    format!("WIPE-{:04X}", (hash & 0xFFFF) as u16)
}
...
// --- M1-7: the executor -------------------------------------------------------
//
// Everything above this point only ever decides whether a write *may* happen.
// This is the first code in this crate that opens a physical device and calls
// raw_write::overwrite. It is reached only from main()'s Ok(bound) arm, after
// confirm_and_bind has already succeeded -- never called speculatively.
//
// unsafe_code stays "forbid": no Win32 FFI in this crate. Exclusive access to
// the target volume(s) is established by a long-lived PowerShell/.NET helper
// (volume_lock module) that performs CreateFile/FSCTL_LOCK_VOLUME/
// FSCTL_DISMOUNT_VOLUME in its own process and holds the lock for the entire
// write -- verified on real hardware that neither removing a drive letter
// nor a short-lived lock+dismount survives past the process that acquired
// it. Opening the physical drive itself uses std::fs::OpenOptions + the safe
// std::os::windows::fs::OpenOptionsExt trait methods (share_mode,
// custom_flags), not a raw CreateFile FFI call.

// # ponytail: FILE_FLAG_WRITE_THROUGH (buffered, but every write forced to
// media before returning) rather than FILE_FLAG_NO_BUFFERING (true direct
// I/O). NO_BUFFERING requires every write buffer to be sector-aligned in
// memory; raw_write::overwrite's plain Vec<u8> chunk buffer isn't, and
// getting an aligned allocation safely in Rust needs unsafe alloc calls --
// exactly what unsafe_code = "forbid" rules out here. Revisit only if a real
// run proves WRITE_THROUGH too slow or shows caching artifacts.
const FILE_SHARE_READ: u32 = 0x0000_0001;
const FILE_SHARE_WRITE: u32 = 0x0000_0002;
const FILE_FLAG_WRITE_THROUGH: u32 = 0x8000_0000;

fn open_physical_drive_for_write(disk_number: u32) -> Result<std::fs::File, String> {
    ...
    std::fs::OpenOptions::new()
        ...
        .open(format!(r"\\.\PhysicalDrive{disk_number}"))
        .map_err(|e| format!("could not open \\\\.\\PhysicalDrive{disk_number}: {e}"))
}

// The only variant that may ever be reported as unqualified success is
// Completed. Everything else exists specifically so a caller can never
// collapse "the requested byte count was written" and "the operation is
...
// Acquires the volume lock (long-lived helper, held for the entire write),
// opens the physical drive, runs the existing tested write loop against it,
// and releases the lock -- combining the write outcome, the lock lifecycle,
// and any flush error into one result that cannot silently claim success.
fn execute_sanitization(bound: &BoundOperation, partitions: &[Partition]) -> SanitizationResult {
    let DiskPlan::Planned {
        ...
        return SanitizationResult::LockNeverEstablished(
            "cannot execute a plan that is not Planned".to_string(),
        );
    };

    let drive_letters: Vec<char> = partitions
        .iter()
        .filter(|p| p.disk_number == bound.disk.number)
        .filter_map(|p| p.drive_letter)
        .collect();

    let mut helper = match volume_lock::acquire(&drive_letters) {
        Ok(helper) => helper,
        Err(e) => return SanitizationResult::LockNeverEstablished(e.to_string()),
    };

    let mut device = match open_physical_drive_for_write(bound.disk.number) {
        Ok(device) => device,
        Err(e) => {
            let lifecycle = volume_lock::release(helper);
            return SanitizationResult::LockNeverEstablished(format!(
                "{e} (lock released: {lifecycle:?})"
            ));
        }
    };

    let outcome = {
        let helper_cell = std::cell::RefCell::new(&mut helper);
        let cancel = || !helper_cell.borrow_mut().is_alive();
        raw_write::overwrite(
            &mut device,
            total_bytes,
            pattern,
        ...
        // helper_cell's borrow of `helper` ends here, before release() needs
        // to consume `helper` by value below.
    };
    let helper_died_mid_write = !helper.is_alive();

    // sync_all() must happen while the helper still holds the locked/
    // dismounted volume -- flushing after release() would let the final
    // flush race against Windows remounting the volume the instant the lock
    ...
    if lifecycle != volume_lock::LockLifecycle::CleanRelease {
        warnings.push(format!("lock cleanup was not clean: {lifecycle:?}"));
    }
    ...
    match outcome {
        ...
        raw_write::WriteOutcome::Cancelled { .. } => SanitizationResult::Cancelled {
            bytes_written,
            reason: if helper_died_mid_write {
                "volume-lock helper was lost mid-write".to_string()
            } else {
                "cancelled".to_string()
            },
            verification,
...
be correlated to a physical disk: {}",
    ...
    let initial_plan = plan_for_disk(selected);
    let (confirmed_plan, capacity_override) = match &initial_plan {
        DiskPlan::Planned { .. } => (initial_plan.clone(), false),
        ...
        DiskPlan::PendingCapacityConfirmation(reason) => {
            println!();
            println!("Disk {} capacity is not trusted: {}", disk_number, reason);
            println!(
                "Get-Disk reports {} bytes exact ({:.2} GB) for this disk.",
                selected.size_bytes, selected.size_gb
            );
            println!();
            println!("This tool will not normally plan a wipe for this bus type, because");
            println!("cheap/counterfeit bridge chips on this bus class are known to");
            println!("misreport their true capacity. You may override this ONLY if you");
            println!("have independently verified the true capacity of this exact device");
            println!("through a means outside this tool.");
            println!();
            println!("To override and proceed with the reported byte count above, type");
            println!("exactly: I VERIFIED THE CAPACITY");
            println!("Anything else cancels. Nothing has been written to any disk yet.");
            println!();
            let ack = read_stdin_line();
            if ack.trim() != "I VERIFIED THE CAPACITY" {
                eprintln!("Capacity override not confirmed. No data has been modified.");
                std::process::exit(2);
            }
            (plan_for_disk_with_capacity_override(selected, true), true)
        }
    };
    let (pattern, chunk_size) = match &confirmed_plan {
        ...
        _ => unreachable!("confirmed_plan is always Planned by this point"),
    };
    ...
    let serial = selected.serial_number.as_deref().unwrap_or("(none)");
    ...
    if capacity_override {
        println!("  Capacity trust: OVERRIDDEN by operator acknowledgment above");
    }
    ...
    println!();
    println!("To proceed, type the confirmation code below exactly and press Enter.");
    ...
    match confirm_and_bind(
        ...
        Ok(bound) => {
            ...
            // Fresh partitions, not the ones fetched at the top of main(): the
            // gap between then and now is exactly when a drive letter could
            // have changed, and the volume-lock helper must act on what's
            // mounted right now, not what was mounted at program start.
            let fresh_partitions = match get_partitions() {
                Ok(partitions) => partitions,
                Err(e) => {
                    eprintln!(
                        "Cannot execute: fresh partition information unavailable: {}",
                        e
                    );
                    eprintln!("No data has been modified.");
                    std::process::exit(2);
                }
            };
...
mod tests {
    ...
    #[test]
    fn confirm_and_bind_rejects_wrong_confirmation_code() {
        let selected = make_target_disk();
        let confirmed_plan = plan_for_disk(&selected);
        let fresh_disks = vec![make_target_disk()];
    ...
    fn confirm_and_bind_rejects_empty_confirmation() {
        ...
        // never coincidentally equal the generated code.
        let selected = make_target_disk();
        let confirmed_plan = plan_for_disk(&selected);
        let fresh_disks = vec![make_target_disk()];
    ...
    fn confirm_and_bind_rejects_a_disk_with_no_serial() {
        // The confirmation code itself matches here -- this must fail via
        // verify_target_before_operation's own non-empty-serial requirement,
        // not the (now code-based, not serial-based) confirmation check.
    ...
    #[test]
    fn confirm_and_bind_accepts_matching_capacity_override_reclassification() {
        // Mirror of the rejection above: when capacity_override is true on
        // both sides (as main() always applies it identically to the
        // original and the re-derived plan), an untrusted-bus disk that
        // stays untrusted must still bind successfully.
        let mut selected = make_target_disk();
        selected.bus_type = Some("USB".to_string());
        let confirmed_plan = plan_for_disk_with_capacity_override(&selected, true);
        assert!(matches!(confirmed_plan, DiskPlan::Planned { .. }));
        ...
        let bound = result.expect("matching capacity-overridden confirmation should bind");
        assert_eq!(bound.plan, confirmed_plan);
    }
    ...
    #[test]
    fn generate_confirmation_code_matches_expected_shape() {
        let code = generate_confirmation_code();
        let suffix = code
            .strip_prefix("WIPE-")
            .expect("code must start with WIPE-");
    ...
    #[test]
    fn generate_confirmation_code_is_not_constant() {
        let codes: std::collections::HashSet<String> =
            (0..20).map(|_| generate_confirmation_code()).collect();
        assert!(
            codes.len() > 1,
            "expected at least some variation across repeated calls"
        );
    }

    // Manual-only, real-hardware dry run of the full pre-write lifecycle
    // against the real Disk 1: spawns the volume-lock helper, waits for
    // LOCKED, opens the physical drive, confirms the helper is still alive
    // while that handle is held, releases the lock, and confirms clean
    // cleanup -- all without ever calling raw_write::overwrite or writing a
    // single byte. #[ignore] so it never runs under plain `cargo test`;
    // requires an elevated shell and the disk present.
    // Run explicitly: `cargo test -- --ignored dry_run_disk_1`.
    #[test]
    #[ignore]
    fn dry_run_disk_1_lock_and_open_without_writing() {
        let partitions = get_partitions().expect("get_partitions should succeed elevated");
        let drive_letters: Vec<char> = partitions
            .iter()
            .filter(|p| p.disk_number == 1)
            .filter_map(|p| p.drive_letter)
            .collect();

        // (a) + (b) + (c): helper starts, and acquire() only returns Ok once
        // every requested letter is locked and dismounted.
        let mut helper = volume_lock::acquire(&drive_letters).expect("lock should succeed");

        // (d): Rust can open the physical drive while the lock is held.
        let device = open_physical_drive_for_write(1).expect("open should succeed while locked");

        // (e): the helper is still alive right up to the point a write would
        // start -- proves the lock genuinely survives past its own spawn,
        // unlike Remove-PartitionAccessPath or a short-lived lock+dismount.
        assert!(
            helper.is_alive(),
            "helper must still be running while the physical-drive handle is held"
        );

        drop(device); // no write() call anywhere in this test

        // (f): release/cleanup works.
        let lifecycle = volume_lock::release(helper);
        assert_eq!(
            lifecycle,
            volume_lock::LockLifecycle::CleanRelease,
            "expected a clean release, got {:?}",
            lifecycle
        );
    }
